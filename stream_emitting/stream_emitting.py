...
def main(args):
    ...
    try:
        ...
        topic = NewTopic(
            name=args.topic_name, num_partitions=1, replication_factor=1
        )
    ...
    records = df[
        ["driver_id", "datetime", "created", "conv_rate", "acc_rate"]
    ].to_dict("records")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--topic_name", default="drivers", help="Kafka topic name"
    )
    parser.add_argument(
        "-d",
        "--data",
        default="data/driver_stats_stream.parquet",
